...
`RING` / `+CLIP` 是纯实时主动上报,模组里没有任何存储——那一刻没人接就永久消失。桥固件监听这些上报并写进自己的环形缓冲,Simplus 从一个独立的有界读操作把它们取走,记成通话历史。

这是**通知**,不是通话能力:Simplus 知道谁在什么时候打过来,不能接听、不能拒接、不能拨出。agentapi 上只有一条 `POST /v1/calls/events` 的读操作,没有任何改变通话状态的路由。

### 来电事件端点

```http
GET {base}/events/calls?after=<seq>&limit=<n>
  -> 200 {"bootId":"0f3a1c2b4d5e6f70","latestSequence":12,"oldestSequence":1,"uptimeMs":39460,
          "events":[{"sequence":12,"number":"1581...","observedAt":1756800000,"observedMs":38210}]}
```
...
- **`bootId` 变了 → 游标归零并从头重读一次。** 游标必须在知道 `bootId` 之前就发出去,所以用旧游标过滤会静默漏掉新一次启动的头几条;发现不一致后重读一次,把代价限制在「每次桥重启一次」,而不是永远每轮都拉整个环。
- **SIM 指纹变了 → 游标跳到 `latestSequence`,不重放。** 环比 SIM 更换活得久,里面剩下的事件是在上一张卡下到达的;记到当前 Line 上等于伪造从未发生过的历史。这条跳过也是**替代**「用 AT 校验订阅身份」的手段——那样每 2 秒就要占用一次模组操作闸门,会与发短信争抢。
- **号码不可用(withheld / 占位文本)→ 计入降级、不建记录、但照样推进游标。** 一条无法归属的事件不能把它后面所有事件永久堵住,与「无法解码的短信降级而不是让整批失败」是同一个判断。
- **写库失败是唯一不推进游标的失败。**
- **丢失量只产生一条运维告警,永不产生通话记录。** 一条没有号码没有时间的记录与真实漏接无法区分,会污染这个功能本身要产出的数据。
- **记录形态是终态。** `direction=inbound`、`state=ended`、`end_reason=CALL_NOT_ANSWERED`、`answered_at` 为空。存成活动通话会让 Line 看起来占线(下一通被 `ErrLineBusy` 拒掉),还会被重启时的对账扫描刷成 `failed`。
- 所有告警**只带计数,不带主叫号码**。号码属于它产出的通话历史,不属于日志。

本地直连的模组没有环形缓冲(它的 `+CLIP` 在任何东西能轮询之前就消失了),agent 会明确报「不支持」而不是返回空页——空页与「没有来电」无法区分。这条路径下 Simplus 不会反复重试和刷日志。
